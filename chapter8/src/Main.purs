...
import Data.AddressBook.Validation (Errors, validatePerson', ValidationError(..))
...
addressBook = createClass $ spec initialState \ctx -> do
  ...
  let renderValidationError err = D.li' [ D.text err ]
      ...
      renderValidationError2 err =
        D.div [ P.className "alert alert-danger" ] [ D.text err ]
      renderValidationError3 (ValidationError err field) =
        D.div [ P.className "alert alert-danger" ] [ D.text err ]
      renderValidationErrors [] = []
      renderValidationErrors xs =
--        [ D.div [ P.className "alert alert-danger" ]
--                [ D.ul' (map renderValidationError xs) ]
          (map renderValidationError3 xs)
--        ]
