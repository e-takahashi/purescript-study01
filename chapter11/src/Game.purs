...
type Game = ExceptT Errors (RWS GameEnvironment Log GameState)
...
{-
game ["cheat1"] = do
  GameState state0 <- get
  for_ (M.keys state0.items) \k -> do
    GameState state <- get
    case M.lookup k state.items of
      Just items -> do
        let newItems = M.delete k state.items
            newInventory = S.union items state.inventory
        put $ GameState state { items     = newItems
                              , inventory = newInventory
                              }
        tell (map (\item -> "You got the " <> show item <> ".") (S.toUnfoldable items :: L.List GameItem))
      _ -> tell (L.singleton "error")
-}
...
game ["cheat"] = do
  ...
  if env.cheatMode
    then do
    GameState state <- get
    let newItems     = M.empty
        newInventory = foldl (\acc k ->
                               case M.lookup k state.items of
                                 Just items -> S.union items acc
                                 _ -> acc
                             ) state.inventory (M.keys state.items)
    put $ GameState state { items     = newItems
                          , inventory = newInventory
                          }
    tell (map (\item -> "You got the " <> show item <> ".") (S.toUnfoldable newInventory :: L.List GameItem))
